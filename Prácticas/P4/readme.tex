...
\begin{center}
\large{\textbf{Práctica 4: Analizadores sintácticos con BYACC/J (YACC).}}
\end{center}
...
\vspace*{1cm}%%%
\begin{center}
    \textbf{\large{21 de Octubre del 2024}}
\end{center}
...
\begin{itemize}
    \item $E\rightarrow E+F|E-F|F$
    \item $F\rightarrow F*G|F/G|G$
    \item $G\rightarrow (E)|id|num$
\end{itemize}
...
Las producciones $Ds, L\_V, Ss, E, F$ tienen recursividad izquierda, por lo que hacemos nuevas producciones en base al algoritmo de la eliminación de la recursividad izquierda:
\begin{itemize}
    ...
    $E'\rightarrow +F E'|-F E'|\epsilon$
    \item $F\rightarrow GF'$\\
    $F'\rightarrow *G F'|/ G F'|\epsilon$
\end{itemize}
...
\begin{itemize}
    ...
    F\rightarrow G F'\\
    F'\rightarrow *G F'|/G F'|\epsilon\\
    G\rightarrow (E)|id|num\}$
\end{itemize}
{\bfseries \item \boldmath Realizar cualquier otro tratamiento necesario para evitar conflictos de shift/reduce mostrando el proceso.}
{\bfseries \item \boldmath Crear una definición con BYACC/J para la gramática resultante.}
\begin{lstlisting}
